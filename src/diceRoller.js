...
// to fix this, we're putting the environment and dice in different collision groups
let diceGroup = 1; 
let envGroup = 2; 
...
function createFloor() {
    ...
    // Rotate the floor body to match the floor mesh
    floorBody.quaternion.setFromAxisAngle(new CANNON.Vec3(1, 0, 0), -Math.PI / 2);

    // collision groups
    floorBody.collisionFilterGroup = envGroup;
    floorBody.collisionFilterMask = diceGroup;
...
function createWalls() {
    createWall(0, 0, -50, 100, 100, 1); // Back wall
    createWall(0, 0, 50, 100, 100, 1); // Front wall
    createWall(-50, 0, 0, 1, 100, 100); // Left wall
    createWall(50, 0, 0, 1, 100, 100); // Right wall
}
...
function rollDice() {
    ...
    if (diceBoxBody && physicsWorld.bodies.includes(diceBoxBody)) {
        diceBoxBody.world.remove(diceBoxBody);
    }
...
function clearDice() {
    ...
    for (let die of dice) {
        if (die && die.body && die.body.world) {
            scene.remove(die);
            die.body.world.remove(die.body);
        }
    }
...
function d4Text(die) {
    ...
    loader.load('helvetiker_bold.typeface.json', function(font) {
        ...
        let tetrahedronGeometry = new THREE.TetrahedronBufferGeometry(params.radius);
        ...
        faceVertices.forEach((vertices, i) => {
            faceNumbers[i].forEach((number, j) => {
                let textGeometry = new THREE.TextGeometry(String(number), {
                    font: font,
                    size: 0.4,
                    height: 0.01,
                });
...
function d4Text2(die) {
    ...
    loader.load('helvetiker_bold.typeface.json', function(font) {
        ...
        faceVertices.forEach((vertices, i) => {
            let textGeometry = new THREE.TextGeometry(String(faceNumbers[i]), {
                font: font,
                size: 0.5,
                height: 0.01,
            });
...
function d6Text(die) {
    ...
    loader.load('helvetiker_bold.typeface.json', function(font) {
        ...
        for (let i = 1; i <= 6; i++) {
            let textGeometry = new THREE.TextGeometry(String(i), {
                font: font,
                size: 0.6,
                height: 0.1,
            });
...
function d12Text(die) {
    ...
    loader.load('helvetiker_bold.typeface.json', function(font) {
        ...
        let dodecahedronGeometry = new THREE.DodecahedronBufferGeometry(params.radius+0.05);
        ...
        for (let i = 0; i < faceCenters.length; i++) {
            let textGeometry = new THREE.TextGeometry(String(i + 1), {
                font: font,
                size: 0.5,
                height: 0.1,
            });
...
function d20Text(die) {
    ...
    loader.load('helvetiker_bold.typeface.json', function(font) {
        ...
        let icosahedronGeometry = new THREE.IcosahedronBufferGeometry(params.radius+0.05);
...
// Convert the BufferGeometry to a ConvexPolyhedron for cannon
...
// function to convert a three.js mesh to a cannon.js shape so that dice collisions do not break physics - not used currently
